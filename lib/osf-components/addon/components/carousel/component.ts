...
export default class Carousel extends Component {
    ...
    // @action
    setUpLiveRegion(region: string) {
        const message = this.intl.t(region + 'new-home.voice_over.live_region.register_keyboard.message');
        const homeMenu = document.getElementById('menu');
        const liveRegion = document.createElement('div');
        liveRegion.setAttribute('aria-live', 'polite');
        liveRegion.setAttribute('aria-atomic', 'true');
        liveRegion.setAttribute('id', 'liveRegion');
        liveRegion.setAttribute('class', 'live-region visually-hidden');

        // get active region, voice help for that region
        if (homeMenu && liveRegion) {
            const subject = 'You';
            const msg: string = subject + message;
            const data = document.createTextNode(msg);
            liveRegion.appendChild(data);
            homeMenu.appendChild(liveRegion);
        }
    ...
    registerKeyboard() {
        ...
        document.addEventListener('keydown', event => {
            ...
            // current wirings are for left, right and dot navigation
            try {
                if (event.ctrlKey) {
