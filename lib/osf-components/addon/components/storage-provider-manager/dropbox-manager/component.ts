...
export default class DropboxManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<DropboxFile | DropboxProviderFile> = [];
    @tracked displayItems: DropboxFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: DropboxFile;
    @tracked selectedFiles: DropboxFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: DropboxFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: DropboxFile, event: PointerEvent) {
