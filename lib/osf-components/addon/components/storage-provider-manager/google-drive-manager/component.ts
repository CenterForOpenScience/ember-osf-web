...
export default class GoogleDriveManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<GoogleDriveFile | GoogleDriveProviderFile> = [];
    @tracked displayItems: GoogleDriveFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: GoogleDriveFile;
    @tracked selectedFiles: GoogleDriveFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: GoogleDriveFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: GoogleDriveFile, event: PointerEvent) {
