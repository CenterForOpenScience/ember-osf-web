...
export default class OwnCloudManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<OwnCloudFile | OwnCloudProviderFile> = [];
    @tracked displayItems: OwnCloudFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: OwnCloudFile;
    @tracked selectedFiles: OwnCloudFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: OwnCloudFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: OwnCloudFile, event: PointerEvent) {
