...
export default class GithubManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<GithubFile | GithubProviderFile> = [];
    @tracked displayItems: GithubFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: GithubFile;
    @tracked selectedFiles: GithubFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: GithubFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: GithubFile, event: PointerEvent) {
