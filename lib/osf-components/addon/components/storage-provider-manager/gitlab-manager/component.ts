...
export default class GitlabManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<GitlabFile | GitlabProviderFile> = [];
    @tracked displayItems: GitlabFile[] = [];
    @tracked filter = '';
    ...
    @task
    @waitFor
    async goToFolder(folder: GitlabFile) {
        this.filter = '';
