...
export default class DataverseManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<DataverseFile | DataverseProviderFile> = [];
    @tracked displayItems: DataverseFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: DataverseFile;
    @tracked selectedFiles: DataverseFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: DataverseFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: DataverseFile, event: PointerEvent) {
