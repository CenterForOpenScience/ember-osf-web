...
export default class OneDriveManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<OneDriveFile | OneDriveProviderFile> = [];
    @tracked displayItems: OneDriveFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: OneDriveFile;
    @tracked selectedFiles: OneDriveFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: OneDriveFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: OneDriveFile, event: PointerEvent) {
