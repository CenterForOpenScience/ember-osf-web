...
export default class BitbucketManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<BitbucketFile | BitbucketProviderFile> = [];
    @tracked displayItems: BitbucketFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: BitbucketFile;
    @tracked selectedFiles: BitbucketFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: BitbucketFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: BitbucketFile, event: PointerEvent) {
