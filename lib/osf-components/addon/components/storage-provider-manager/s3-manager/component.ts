...
export default class S3Manager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<S3File | S3ProviderFile> = [];
    @tracked displayItems: S3File[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: S3File;
    @tracked selectedFiles: S3File[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: S3File) {
        this.filter = '';
    ...
    @action
    selectFile(file: S3File, event: PointerEvent) {
