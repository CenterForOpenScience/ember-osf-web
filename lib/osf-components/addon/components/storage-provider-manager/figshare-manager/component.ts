...
export default class FigshareManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<FigshareFile | FigshareProviderFile> = [];
    @tracked displayItems: FigshareFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: FigshareFile;
    @tracked selectedFiles: FigshareFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: FigshareFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: FigshareFile, event: PointerEvent) {
