...
export default class BoxManager extends Component<Args> {
    @tracked storageProvider?: FileProviderModel;
    @tracked folderLineage: Array<BoxFile | BoxProviderFile> = [];
    @tracked displayItems: BoxFile[] = [];
    @tracked filter = '';
    @tracked sort = FileSortKey.AscName;
    @tracked currentPage = 1;
    @service currentUser!: CurrentUserService;

    @tracked baseSelectedFile?: BoxFile;
    @tracked selectedFiles: BoxFile[] = [];
    ...
    @task
    @waitFor
    async goToFolder(folder: BoxFile) {
        this.filter = '';
    ...
    @action
    selectFile(file: BoxFile, event: PointerEvent) {
