...
module('Integration | Component | registries-navbar', hooks => {
    ...
    test('it renders', async function(assert) {
        await render(hbs`{{registries-navbar}}`);
    ...
    test('desktop layout', async function(assert) {
        ...
        await render(hbs`{{registries-navbar signUpURL='http://example.com'}}`);
        ...
        assert.dom('[data-test-search-toggle]').isNotVisible('Mobile search toggle not visible on desktop');
    ...
    test('desktop layout (logged out)', async function(assert) {
        ...
        assert.dom('a[data-test-join]').hasAttribute('href', RegExp('^http://example\\.com\\?next='));
    ...
    test('mobile layout', async function(assert) {
        ...
        assert.dom('[data-test-search-bar]').isNotVisible('Search bar hidden visible');
        assert.dom('[data-test-search-toggle]').isVisible('Mobile search toggle visible on mobile');
    ...
    test('onSearch', async function(assert) {
        ...
        await render(hbs`{{registries-navbar onSearch=this.onSearch}}`);
    ...
    test('onSearch (Mobile)', async function(assert) {
        ...
        assert.dom('[data-test-search-bar-mobile]').isNotVisible();

        await click('[data-test-search-toggle]');

        // toggle animated, wait for it to finish
