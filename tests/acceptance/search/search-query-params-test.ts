...
module(moduleName, hooks => {
    ...
    test('default query-parameters', async function(assert) {
        ...
        assert.equal(currentURL(), '/search?resourceType=Project%2CProjectComponent', 'Query-params are updated');
        // change sort
    ...
    test('default query-parameters, mobile', async function(assert) {
        ...
        assert.equal(
            currentURL(),
            '/search?resourceType=Registration%2CRegistrationComponent',
            'Object type query-param updated',
        );
        // change sort
