...
test('file rendering defaults', function(assert) {
    ...
    assert.equal(
        ...
        `${config.OSF.renderUrl}?url=${encodeURIComponent(`${download}?direct&mode=render&initialWidth=766`)}`,
    );
});
...
test('specify file rendering parameters', function(assert) {
    ...
    assert.equal(
        this.$('iframe').attr('src'),
        `${config.OSF.renderUrl}?url=${encodeURIComponent('http://cos.io/?direct&mode=render&initialWidth=766')}`,
    );
});
